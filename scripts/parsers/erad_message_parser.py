...
#Field `ArgumentIsOptionTest.options` can be null but is not declared `@Nullable`. (Origin: null constant at line 24).
#`stripLeadingHyphens(...)` needs a non-null value in parameter 1 but argument `null` can be null. (Origin: null constant at line 32).
#Field `OptionGroup.selected` is not initialized in the constructor and is not declared `@Nullable`.

REGEX_LIST= [
    'Field .* can be null but is not declared .*',
    '.* needs a non-null value in parameter .* but argument .*',
    'Field .* is not initialized in the constructor and is not declared .*',
    '.* needs a non-null value in parameter .* but argument .* can be null .*',
    'Field .* is not initialized in the constructor and is not declared .*',
